...
namespace MonMoose.Battle
{
    public class MovePathFindModule : Module
    {
        ...
        private BattleGrid m_startGrid;
        private BattleGrid m_targetGrid;
        ...
        public bool FindPath(Entity entity, BattleGrid startGrid, DcmVec2 offset, BattleGrid targetGrid, List<BattleGrid> gridList)
        {
            ...
            if (m_offset != DcmVec2.zero)
            {
                ...
                BattleGrid grid = m_battleInstance.GetGrid(GetNearGridPosition(startGrid.gridPosition, m_offset));
                if (grid != null)
                {
        ...
        {
            BattleGrid grid = m_battleInstance.GetGrid(path.grid.gridPosition + new GridPosition(offsetX, offsetY));
        ...
        private void AddToOpenList(MovePath fromPath, BattleGrid toGrid, Dcm32 g)
        {
        ...
        private void CollectPath(List<BattleGrid> gridList)
        {
            MovePath path = m_openList.First.Value;
            while (path != null)
            {
        ...
        private bool CheckInCloseList(BattleGrid grid)
        {
        ...
        {
            MovePath path = m_battleInstance.FetchPoolObj<MovePath>(this);
